...
**Can I run an iOS simulator or Android emulator?**

Yes. A [device session](/bitrise-rde/devices/device-sessions) boots one with the session and streams it to the browser, and a [device preview link](/bitrise-rde/devices/device-preview-links) lets someone without a Bitrise account try a build on one.
...
A personal session is owned by the user who created it and only they can manage it. A workspace-owned session, created with `bitrise-cli rde session create --owner workspace` or spawned by a [device preview link](/bitrise-rde/devices/device-preview-links) minted from CI, is visible to and manageable by every workspace member under **Workspace sessions**.

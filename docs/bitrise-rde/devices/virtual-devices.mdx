...
- A **<GlossTerm baseform="device preview link">device preview link</GlossTerm>** turns a finished app build into a URL. Whoever opens it gets a fresh device with the app installed and running, with no Bitrise login. The typical use is the [Device Preview Step](https://bitrise.io/integrations/steps/device-preview) posting the link on every pull request. See [Device preview links](/bitrise-rde/devices/device-preview-links).

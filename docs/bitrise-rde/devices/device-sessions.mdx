...
On the **New session** form, the device section appears once the selected stack or template supports a device: a macOS stack for iOS, a Linux Android stack for Android.
...
Pass `--device-platform ios` or `--device-platform android`. With `--wait`, the command returns when the device is ready, not just when the machine is running:
...
See [Create session](/bitrise-rde-api/api-reference/codespaces-service-create-session) in the API reference.

...
A <GlossTerm baseform="warm pool">warm pool</GlossTerm> keeps a number of <GlossTerm baseform="rde session">sessions</GlossTerm> booted and idle, so that creating a session takes seconds instead of the minutes a fresh machine needs to boot and run its setup. You describe the session once, the way you would when creating one, and set a pool size. Bitrise keeps that many sessions warm, and hands one out the moment someone asks for it.

Warm pools are made for the moments when waiting hurts: an AI agent that needs a machine now, a team that starts many similar sessions a day, and [device preview links](/bitrise-rde/devices/device-preview-links) where a reviewer clicks and expects the app on screen.
...
Claiming a session from a pool gives you one of its warm sessions, renamed and stamped with your name, labels, and auto-terminate window, already running. If no warm session is available at that moment, a session is created from the pool's configuration instead, so a claim always succeeds. Every session records where it came from: a claimed session's warm state is `claimed`, a session created on demand from the pool is `cold`.
...
A session claimed from a personal pool is owned by you, like a session you create yourself. A session claimed from a workspace pool is owned by the workspace: every member sees and manages it under **Workspace sessions**, and you're recorded as its creator.
...
1. Give the pool a name and choose the owner: **Me** for a personal pool or **Workspace** for a shared one.
...
The configuration is validated the same way a session create is: an unknown input, a missing required input, or a device the machine type can't run is rejected when you save. Each workspace has a limit on the size of one pool and on the total across its pools; the form and the CLI tell you the limit when you exceed it.

...
## Setting up on non-Bitrise CIs

- [Build Cache for Gradle](/bitrise-build-cache/build-cache-for-gradle/configuring-the-build-cache-for-gradle-in-non-bitrise-ci-environments).
- [Build Cache for Bazel](/bitrise-build-cache/build-cache-for-bazel/configuring-the-build-cache-for-bazel-in-non-bitrise-ci-environments).
...
On GitHub Actions, follow the dedicated GitHub Actions guide for your build system instead. Jobs that run on [Bitrise Build Hub](/bitrise-build-hub/build-hub-for-github-actions/build-hub-for-github-actions-overview) runners need no token: the machine is already connected to your Bitrise Workspace, so the CLI authenticates itself.

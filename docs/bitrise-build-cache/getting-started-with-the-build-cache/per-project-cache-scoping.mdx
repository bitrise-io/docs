...
description: "Turn Build Cache activation into a per-project opt-in by marking the projects that should use the cache and gating the rest."
sidebar_position: 8
...
Re-run the interactive wizard and pick opt-in at the **Cache for all projects, or only opted-in ones?** step:
...
`bitrise-build-cache doctor` reports the effective mode and whether the current directory would be gated:

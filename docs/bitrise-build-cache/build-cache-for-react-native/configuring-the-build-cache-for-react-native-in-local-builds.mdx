...
If your team already populates the cache from CI and you want your local builds to only read from it, turn cache push off. A broken local build then can't affect anyone else — the trade-off is that your local builds contribute nothing, so if CI doesn't run yet the Android build keeps reporting `0 from cache` and the iOS build `0 / N (0%)`.

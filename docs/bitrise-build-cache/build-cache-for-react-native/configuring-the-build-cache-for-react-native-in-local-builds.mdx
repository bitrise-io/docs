...
1. **Cache for all projects, or only opted-in ones?**: keep the default **Always** unless several projects live on this machine and you only want a subset to use the cache. See [Per-project cache scoping](/bitrise-build-cache/getting-started-with-the-build-cache/per-project-cache-scoping).

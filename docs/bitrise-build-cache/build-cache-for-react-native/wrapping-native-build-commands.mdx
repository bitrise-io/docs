...
- Tracks cache hit rates and reports build analytics back to <NT>Bitrise</NT>.
...
You only need to wrap commands that ultimately invoke <NT>Gradle</NT> or <NT>Xcode</NT>. Plain JS commands (`yarn install`, `yarn test`, etc.) do not need to be wrapped.

...
- `--remote_upload_local_results` is push mode, the default. Activating with `--cache-push=false` writes `--noremote_upload_local_results` instead.
...
The setup in this guide activates the Build Cache with cache push enabled: your local builds read from and write to the shared cache. Every action your machine runs becomes an entry your teammates and CI can reuse, and your activity shows up on the [Build Cache page](https://app.bitrise.io/build-cache/).
...
### Force pull-only for developers who don't run the activation

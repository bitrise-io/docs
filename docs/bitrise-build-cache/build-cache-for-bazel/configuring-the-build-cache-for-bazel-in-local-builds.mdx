...
If your team already populates the cache from CI and you want your local builds to only read from it, turn cache push off. A broken local build then can't affect anyone else — the trade-off is that your local builds contribute nothing, so if CI doesn't run yet the summary line keeps reporting `0 remote cache hit`.

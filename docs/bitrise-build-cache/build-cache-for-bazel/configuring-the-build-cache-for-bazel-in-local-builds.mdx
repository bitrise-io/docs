...
You can use the <NT>Bitrise</NT> <NT>Build Cache for Bazel</NT> on any machine: you just need to create a `bitrise.bazelrc` configuration file that includes the required configuration for the cache endpoints.
...
That's it! You can now run any `bazel` commands and take advantage of the <NT>Bitrise</NT> <NT>Build Cache</NT>. You can check the <NT>invocation details</NT> through the link printed during builds:

...
[Bitrise Insights](/insights) offers detailed metrics on the performance of your Build Cache, including invocation count, command duration, or cache hit rate. You can explore Insights starting from any invocation:

1. Open the Build Cache.
1. Open the [Invocation details](/bitrise-build-cache/getting-started-with-the-build-cache/invocations) page.
...
For another example, here's a sudden change in cache hit rate for a given <GlossTerm baseform="workflow">Workflow</GlossTerm>, suggesting a weak spot in the caching setup:

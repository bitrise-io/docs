...
CodePush code signing is done in three <GlossTerm baseform="stage">stages</GlossTerm>:
...
CodePushNext and the Bitrise CodePush CLI both support code signing for Expo <GlossTerm baseform="project">projects</GlossTerm>, too. The overall flow is the same as for standard React Native apps: generate an RSA key pair, embed the public key in the app, and sign bundles at release time using the --private-key-path flag.

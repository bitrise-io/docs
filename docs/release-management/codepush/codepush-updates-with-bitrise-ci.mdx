...
- Creating a <GlossTerm baseform="workflow">Workflow</GlossTerm> that creates the update packages for both iOS and Android, and updates both to the Bitrise CodePush Server.
...
1. [Create Secrets](/en/bitrise-ci/configure-builds/<GlossTerm baseform="secret">secrets</GlossTerm>#setting-a-secret) for the deployment key and the API token:

...
description: "Initialize and run a CodePush project after successfully installing the CodePush CLI."
sidebar_position: 4
---
...
After successfully installing the CodePush CLI and setting up authentication, you can start a new project. To start the project, run the following command:
...
Another option is to set the $CODEPUSH_APP_ID Environment Variable. If the app ID is available from more than one source, the CodePush CLI resolves them in the following order:
...
To update your app via CodePush, you need to create an update bundle and then push it to the CodePush server. The CodePush CLI has two commands for this purpose:
...
The CLI also allows you to list deployments, view their details and release history. You can also clear all releases from a deployment.
...
Use the CodePush CLI to manage updates for your app. You can view update details, check processing status, and remove updates.

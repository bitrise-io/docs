---
title: "CodePush CLI authentication"
sidebar_position: 2
---
...
To interact with the <NT>Bitrise</NT> API, which is required for CodePush commands, you need an API token. The token can be accessed in two ways:
...
1. When running a command, the CodePush CLI resolves the Env Var automatically.
...
1. Run one of the following commands:
   ...
   - <NT>Linux</NT>: `~/.config/codepush/config.json`

## Revoking a locally stored token

If you want to revoke an API token that you stored locally for CodePush, you can do so at any time. If you revoke the token, you won't be able to interact with the <NT>Bitrise</NT> API unless you set the BITRISE_API_TOKEN Environment Variable.

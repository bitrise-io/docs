...
The CodePush CLI can be installed in two ways: as a <NT>Bitrise CLI</NT> plugin, or as a standalone binary. Both support the same commands. Use plugin mode if you already use the <NT>Bitrise CLI</NT>; use standalone mode if you want to run the CLI independently.
...
You can manage the plugin with standard <NT>Bitrise CLI</NT> commands:
...
In standalone mode, commands are prefixed with `codepush`. The <NT>Bitrise CLI</NT> is not required.
...
1. Download the binary for your platform from the [Releases page](https://github.com/bitrise-io/bitrise-plugins-codepush-cli/releases):
   ...
   | Platform | Binary |
   |---|---|
   | macOS (<NT>Apple</NT> Silicon) | `codepush-Darwin-arm64` |
   | macOS (Intel) | `codepush-Darwin-x86_64` |
   | <NT>Linux</NT> (x86_64) | `codepush-Linux-x86_64` |
...
Set a different target environment for CodePush instead of the main <NT>Bitrise</NT> server. For example, you can set a staging environment as the base server URL.

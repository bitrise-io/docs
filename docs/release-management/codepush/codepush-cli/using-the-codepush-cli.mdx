...
For a quick overview of all commands, flags, and environment variables, see the [CodePush CLI reference](/release-management/codepush/codepush-cli/codepush-cli-reference).
...
The CLI generates source maps by default and writes them to `<output-dir>.sourcemaps/<bundle-name>.map`, outside the bundle directory that `push` uploads. To write them somewhere else, use `--sourcemap-output` (`-s`). The path must be outside the output directory:
...
When you bundle and push in one step, `push --bundle` accepts the same `--sourcemap` and `--sourcemap-output` flags.

...
If you already have webhooks registered to [automatically trigger builds](/en/bitrise-ci/run-and-analyze-builds/build-<GlossTerm baseform="trigger">triggers</GlossTerm>/configuring-build-triggers) on Bitrise, check that their permissions match the requirements of Git Insights and update them if necessary.
...
- **[GitLab](https://docs.gitlab.com/ee/user/<GlossTerm baseform="project">project</GlossTerm>/integrations/webhooks.html#configure-a-webhook-in-gitlab)**: Push events/All branches, Merge request events.
...
1. Log in to Bitrise.
1. Select a <GlossTerm baseform="workspace">Workspace</GlossTerm> in the top right corner of the Dashboard.

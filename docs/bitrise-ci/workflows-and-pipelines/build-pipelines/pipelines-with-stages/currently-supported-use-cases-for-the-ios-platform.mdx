...
This example uses the [sample-swift-project-with-parallel-ui-test](https://github.com/bitrise-io/sample-swift-<GlossTerm baseform="project">project</GlossTerm>-with-parallel-ui-test) iOS Open Source sample app, which has some example Unit and UI tests and uses Test Plans to group the tests.

The example <GlossTerm baseform="pipeline">Pipeline</GlossTerm> config showcases how to run all the test cases of the project on different iOS simulators.

`run_tests_on_simulators` Pipeline runs two <GlossTerm baseform="stage">Stages</GlossTerm> sequentially:

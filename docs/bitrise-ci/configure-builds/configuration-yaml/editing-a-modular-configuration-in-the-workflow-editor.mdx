...
A module tab shows a single file. What you can do there depends on where that file comes from. Modules in your own repository on the current branch are fully editable. Modules pulled in from another repository, branch, tag, or commit load read-only. Inside an editable module, entities that the file only refers to are read-only as well: a <GlossTerm baseform="workflow">Workflow</GlossTerm> defined in another module shows up here, but you edit its definition in the module that owns it.
...
- You can't edit modules from another repository, branch, tag, or commit. They load so you can read them, but they can't be edited.

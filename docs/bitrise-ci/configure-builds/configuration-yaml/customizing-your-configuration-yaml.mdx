...
Any tool that can edit a <GlossTerm baseform="configuration yaml">configuration YAML</GlossTerm> file can add custom properties to it. This way you can add special properties or notes to your [Env Vars](/en/bitrise-ci/configure-builds/environment-variables), or even try new configurations of your <GlossTerm baseform="Workflow">Workflow</GlossTerm>. All you have to add is add a `meta` field and a namespace label with key and value to the right place.
...
**Using your own version of the <GlossTerm baseform="workflow editor">Workflow Editor</GlossTerm>**

You can fork our <GlossTerm baseform="Workflow Editor">Workflow Editor</GlossTerm> and use your own version of it. Then you can use `meta`: for example, let's say you want to keep an eye on one of the Environment Variables (Env Var), you want to know when it was last modified and by who.
...
When you change the <GlossTerm baseform="stack">stack</GlossTerm> configuration on the <GlossTerm baseform="workflow">Workflow</GlossTerm> Editor UI, you modify `meta`under the hood. In this example, the `deploy` Workflow is configured to run on Xcode 16.1.

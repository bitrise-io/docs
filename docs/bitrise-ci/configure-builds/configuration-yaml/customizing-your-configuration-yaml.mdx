...
Any tool that can edit a configuration YAML file can add custom properties to it. This way you can add special properties or notes to your [Env Vars](/en/bitrise-ci/configure-builds/environment-variables.html), or even try new configurations of your <GlossTerm baseform="Workflow">Workflow</GlossTerm>. All you have to add is add a `meta` field and a namespace label with key and value to the right place.
...
**Using your own version of the Workflow Editor**
...
When you change the stack configuration on the Workflow Editor UI, you modify `meta`under the hood. In this example, the `deploy` Workflow is configured to run on Xcode 16.1.

...
This is a basic reference for the most important aspects of CI/CD configuration in YAML. For the full reference, see [Configuration YAML reference](/bitrise-ci/references/configuration-yaml-reference).

:::

Any tool that can edit a <GlossTerm baseform="configuration yaml">configuration YAML</GlossTerm> file can add custom properties to it. This way you can add special properties or notes to your [Env Vars](/bitrise-ci/configure-builds/environment-variables), or even try new configurations of your <GlossTerm baseform="Workflow">Workflow</GlossTerm>. All you have to add is add a `meta` field and a namespace label with key and value to the right place.

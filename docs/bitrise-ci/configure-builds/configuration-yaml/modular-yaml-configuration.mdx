...
Modular YAML configuration is only available for <GlossTerm baseform="workspace">Workspaces</GlossTerm> with Enterprise plans. You also need to [store your configuration files in your own repository](/en/bitrise-ci/configure-builds/configuration-yaml/managing-a-<GlossTerm baseform="project">project</GlossTerm>-s-configuration-yaml-file#storing-the-bitriseyml-file-in-your-repository), not on bitrise.io.
...
After an additional YAML file has been included in the `bitrise.yml` file, you can refer to any of its <GlossTerm baseform="workflow">Workflows</GlossTerm> or <GlossTerm baseform="pipeline">Pipelines</GlossTerm> as to any other Workflow or Pipeline in your configuration.
...
You can have a total of 50 configuration files, including the root level `bitries.yml` file.
...
Bitrise needs `read` access to all repositories where <GlossTerm baseform="configuration yaml">configuration YAML</GlossTerm> files are hosted. There are two ways to make sure Bitrise can access your repo:
...
A configuration module is a full YAML configuration file. That means that all configuration elements of a valid `bitrise.yml` file are available: you can define the `format_version`, app level Environment Variables, default <GlossTerm baseform="stack">stacks</GlossTerm> and machine types, and of course Pipelines, <GlossTerm baseform="stage">Stages</GlossTerm>, and Workflows in a configuration module.
...
However, non-root level entities cannot stand alone in a separate module file either: you can't, for example, include <GlossTerm baseform="step">Step</GlossTerm> inputs like this.

...
## Configuring the Bitrise CLI

The CLI saves its settings to `~/.config/bitrise/cli/config.yml`. Make sure `~/.config` (or `$XDG_CONFIG_HOME`, if you set it) is writable. Otherwise, `bitrise local setup` and plugin commands can fail.

To learn about the config files and the settings you can save, see [Configuring the Bitrise CLI](/bitrise-ci/bitrise-cli/configuring-the-bitrise-cli).

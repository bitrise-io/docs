...
Every command acts on one <GlossTerm baseform="project">project</GlossTerm>. Pass it with `--app`, or save a default: see [Selecting the Workspace and the project](/bitrise-ci/bitrise-cli/bitrise-cli-overview#selecting-the-workspace-and-the-project). The examples on this page pass `--app` every time.
...
To print the `bitrise.yml` the build ran with, use `bitrise build yml`. The file can be different from the project's current configuration if someone changed it since the build ran. See [Working with bitrise.yml via the Bitrise CLI](/bitrise-ci/bitrise-cli/working-with-bitrise-yml-via-the-bitrise-cli#downloading-the-bitriseyml-file).

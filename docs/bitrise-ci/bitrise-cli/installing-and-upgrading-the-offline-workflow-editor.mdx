...
## About the offline Workflow Editor

The offline Workflow Editor is a local copy of the Workflow Editor that ships with the Bitrise CLI. It runs in your browser on your own computer and edits your project's <GlossTerm baseform="configuration yaml">configuration YAML</GlossTerm> file locally.
...
1. Run `bitrise local setup` to install offline Workflow Editor as part of the Bitrise Plugins.

   Running `bitrise local setup` also checks if Bitrise Core tools, Bitrise Plugins and Toolkits are installed on your local machine. If not, it will automatically install them. On macOS, it also checks for Homebrew, prompting you to install it manually if missing.

...
A device session is a Remote Dev Environment session that also boots a virtual device — an iOS simulator on a macOS stack or an Android emulator on a Linux stack — reports when it is ready, and streams its screen to the browser.

## Device preview link {#device-preview-link}

A device preview link is a shareable URL that opens an app build on a live iOS simulator or Android emulator in the recipient's browser, without a Bitrise login. Every open starts its own device session, and the link expires after a set lifetime.

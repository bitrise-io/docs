...
You can add new <GlossTerm baseform="project">projects</GlossTerm> on Bitrise with the API: add the <GlossTerm baseform="Project">project</GlossTerm>, generate SSH keys, and set up the project’s initial configuration.
...
1. Register the app by calling the `register` endpoint and setting all required parameters.
   ...
   You need to set your git provider, the repository URL, the slug of the repository as it appears at the provider, and the slug of the owner of the repository. You also need to add the slug of the <GlossTerm baseform="workspace">Workspace</GlossTerm> that will own the app: due to legacy naming conventions, you need the `organization_slug` parameter.

...
You can use the <NT>Bitrise</NT> API to list an app's build, get all information about a specific build, view the build logs, and view archived builds that are older than 200 days.
...
| GET /apps/\{app-slug\}/builds/\{build-slug\}/<NT><GlossTerm baseform="bitrise.yml">bitrise.yml</GlossTerm></NT> | Get the `bitrise.yml` file of one of the builds of a specified app. | Developer |
| GET /apps/\{app-slug\}/builds/\{build-slug\}/log | Get the build log of a specified build of an app. | Developer |
| GET /builds | List all the <NT>Bitrise</NT> builds that can be accessed with the authenticated account. | N/A |
...
You can get all builds of an app with the `GET /apps/{app-slug}/builds` endpoint. You can set <NT>additional parameters</NT>, such as the <NT><GlossTerm baseform="workflow">Workflow</GlossTerm></NT> that was used for the build, to act as filters. Set parameters in the following format:
...
**Listing builds that built the `development` branch with the primary <NT>Workflow</NT>**

...
| GET /apps/\{app-slug\}/builds/\{build-slug\}/<GlossTerm baseform="bitrise.yml">bitrise.yml</GlossTerm> | Get the `<GlossTerm baseform="bitrise.yml">bitrise.yml</GlossTerm>` file of one of the builds of a specified app. | Developer |
...
You can get all builds of an app with the `GET /apps/{app-slug}/builds` endpoint. You can set additional parameters, such as the <GlossTerm baseform="workflow">Workflow</GlossTerm> that was used for the build, to act as filters. Set parameters in the following format:

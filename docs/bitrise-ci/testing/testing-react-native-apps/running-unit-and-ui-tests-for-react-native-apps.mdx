...
<Tabs>
  ...
  1. Write your tests and add them to your React Native <GlossTerm baseform="project">project</GlossTerm>.
  ...
  1. Configure the <GlossTerm baseform="step">Step</GlossTerm> to run the `test` command:
  ...
  1. In the <GlossTerm baseform="configuration yaml">Configuration YAML</GlossTerm> file, add either the `npm` or the `yarn` <GlossTerm baseform="Step">Step</GlossTerm> to your <GlossTerm baseform="Workflow">Workflow</GlossTerm>, depending on which package manager you use in your project.

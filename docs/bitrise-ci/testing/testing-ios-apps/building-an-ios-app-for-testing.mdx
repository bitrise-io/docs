...
1. For code signing, make sure you have connected your [Apple service account](/en/bitrise-platform/integrations/apple-services-connection.html) to Bitrise.
   ...
   :::
1. Make sure [you install all of the app's dependencies](/en/bitrise-ci/dependencies-and-caching/ios-dependencies.html) in your Workflow.
...
1. If you need code signing, we recommend using the [automatic code signing](/en/bitrise-ci/code-signing/ios-code-signing/managing-ios-code-signing-files-automatic-provisioning) option: find the **Automatic code signing** input group, and choose a method from the dropdown menu of the **Automatic code signing method** input.
   ...
   Keep in mind that you need to upload [code signing certificates](/en/bitrise-ci/code-signing/ios-code-signing/managing-ios-code-signing-files-automatic-provisioning/uploading-ios-code-signing-certificates) to Bitrise in order to successfully sign your app.
...
1. Add the **Deploy to Bitrise.io** Step to the end of your Workflow to be able to access the generated artifacts either [on the **Artifacts** tab](/en/bitrise-ci/run-and-analyze-builds/managing-build-files/build-artifacts-online.html) or in the **Tests** tab.
...
1. If you need code signing, we recommend using the [automatic code signing](/en/bitrise-ci/code-signing/ios-code-signing/managing-ios-code-signing-files-automatic-provisioning) option: set the `automatic-code-signing` input to either of two values:
   ...
   `apple-id`: Use [Apple ID authentication](/en/bitrise-platform/integrations/apple-services-connection/connecting-to-an-apple-service-with-apple-id.html) to manage code signing.

   `api-key`: Use [API key authentication](/en/bitrise-platform/integrations/apple-services-connection/connecting-to-an-apple-service-with-api-key.html) to manage code signing.

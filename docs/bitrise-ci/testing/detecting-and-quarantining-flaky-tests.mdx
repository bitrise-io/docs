...
1. **[Android Unit Test](https://bitrise.io/integrations/steps/android-unit-test)**
   ...
   This functionality is configured directly through the [Gradle plugin](https://github.com/gradle/test-retry-gradle-plugin). You can enable retries for failed tests within your <GlossTerm baseform="project">project's</GlossTerm> `build.gradle` file.
...
<Tabs>
  ...
  1. Define the quarantine scope (workflows, <GlossTerm baseform="pipeline">pipelines</GlossTerm>, or branches) by clicking **[Edit quarantine scope](/en/bitrise-ci/testing/detecting-and-quarantining-flaky-tests#defining-the-scope-of-quarantine)**.
...
You can run a fastlane lane in a Bitrise <GlossTerm baseform="workflow">Workflow</GlossTerm> either with the official Bitrise **Fastlane** Step or using a **Script** Step.
...
You can convert the JSON data either in the <GlossTerm baseform="configuration yaml">configuration YAML</GlossTerm> file or in the fastlane config file (`Fastfile`).

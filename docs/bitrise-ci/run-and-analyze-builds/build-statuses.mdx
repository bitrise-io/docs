...
- **Aborted**: A build can be aborted manually by the user, or automatically either by the [Rolling builds feature](/bitrise-ci/configure-builds/configuring-build-settings/rolling-builds) or because your build time has run out.
  ...
  Use the abort_with_success parameter with [a Bitrise API call to abort a build](/bitrise-ci/api/triggering-and-aborting-builds#aborting-a-build) but still count it as a successful one. With the [Bitrise CLI](/bitrise-ci/bitrise-cli/installing-and-updating-the-bitrise-cli), run `bitrise build abort BUILD-SLUG --abort-with-success`.

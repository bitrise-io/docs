...
- **Aborted**: A build can be aborted manually by the user, or automatically either by the [Rolling builds feature](/bitrise-ci/configure-builds/configuring-build-settings/rolling-builds) or because your build time has run out.
  ...
  There is a specific status called Aborted with success: this means the build has been aborted with the API or the Bitrise CLI but it is reported as a success to your git hosting provider.

  Use the abort_with_success parameter with [a Bitrise API call to abort a build](/bitrise-ci/api/triggering-and-aborting-builds#aborting-a-build) but still count it as a successful one. With the Bitrise CLI, run `bitrise build abort BUILD-SLUG --app APP-SLUG --abort-with-success`: see [Aborting a build](/bitrise-ci/bitrise-cli/managing-builds-with-the-bitrise-cli#aborting-a-build).

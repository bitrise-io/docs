...
GitHub stacked pull requests split a change into an ordered series, where each pull request targets the branch of the one below it and the whole series lands on a single base branch.

- Triggers match the target branch condition against the base branch of the stack, not the branch a pull request targets directly. Set up your pull request triggers against the branch the stack lands on, such as `main`.
- `$BITRISEIO_GIT_BRANCH_DEST` holds the base branch of the stack.

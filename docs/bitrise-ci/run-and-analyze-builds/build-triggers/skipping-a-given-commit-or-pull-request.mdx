...
:::caution[Skipped builds and required checks]

Skipping a build means Bitrise reports no status to your Git provider. If you have a required status check in your branch protection rules, a skipped build leaves that check pending and the pull request can't be merged. To get a green required check without running a build, trigger a no-op Pipeline instead: one where every Workflow is skipped. For details, see [Passing a required check without running a build](/en/bitrise-ci/configure-builds/configuring-build-settings/reporting-the-build-status-to-your-git-hosting-provider#passing-a-required-check-without-running-a-build).
...
Yo recommend that you insert below `run_if` command to each Step in your <GlossTerm baseform="workflow">Workflow</GlossTerm> to skip the Steps.

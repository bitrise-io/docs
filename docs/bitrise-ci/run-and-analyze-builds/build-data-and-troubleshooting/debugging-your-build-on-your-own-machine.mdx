...
1. Download your app’s `bitrise.yml` file from [bitrise.io](https://www.bitrise.io/). To get the exact configuration a failed build ran with, run `bitrise build yml BUILD-SLUG --app APP-SLUG > bitrise.yml`: see [Viewing a build](/bitrise-ci/bitrise-cli/managing-builds-with-the-bitrise-cli#viewing-a-build).
1. Run the build with: `bitrise local run <workflow-name>` (for example, `bitrise local run primary`).

...
1. Make sure your local network address space does not clash with the [Bitrise virtual machines' address space](/bitrise-platform/infrastructure/build-machines/configuring-your-network-to-access-our-build-machines) as this can cause an error with the VPN.
...
1. Add a new project, using this repository as the source. Make sure it is a private project! There is no need to register a [webhook](/bitrise-platform/integrations/webhooks/adding-incoming-webhooks).
1. <Partial_GettingToTheProjectSettingsPage />
1. On the left, select **Repository** and click **Change**.
...
1. Register the encoded certificates and the key as [Secrets](/bitrise-ci/configure-builds/secrets).
...
1. Provide VPN client settings and credentials required for the Step either by:
   ...
   - Using the **Commandline options** input of the Step. The options specified in this input will take precedence over the configuration file!

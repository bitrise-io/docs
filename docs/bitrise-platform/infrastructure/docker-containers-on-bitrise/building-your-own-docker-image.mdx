...
To speed up your builds, the Step also supports [key-based caching](/bitrise-ci/dependencies-and-caching/key-based-caching/accessing-key-based-cache-archives).
...
1. Optionally, customize the build command with build arguments and options.
   ...
   - Add [options](https://docs.docker.com/engine/reference/commandline/image_build/#options) in the **Extra options** input: one extra option per line in the format of `--option value` or `--option=value`.
     ...
     :::
1. Optionally, set the **Enables to use the host network with the buildkit build container** input to **true** to let the build container use the host's network. Set the **Verbose logging** input to **true** for additional troubleshooting output.
...
1. Optionally, customize the build command with build arguments and options.
   ...
   ```
1. Optionally, set the `buildx_host_network` input to **true** to let the build container use the host's network. Set the `verbose` input to **true** for additional troubleshooting output.

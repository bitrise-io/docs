...
# Install <NT>Bitrise MCP</NT> Server in Cursor

## Prerequisites

1. [Cursor](https://cursor.com/download) IDE installed (<NT>latest version</NT>, with MCP OAuth support — Cursor v0.48.0+ for Streamable HTTP, recent builds for OAuth)
2. A <NT>Bitrise</NT> account
3. For local setup: [Go](https://go.dev/) (>=1.25) installed and a <NT>Bitrise</NT> PAT

## Remote Server Setup (Recommended)

Recent Cursor versions support MCP OAuth — on first tool use Cursor opens your browser to sign in to <NT>Bitrise</NT>; no token to paste.
...
The local <NT>Bitrise MCP</NT> server runs via Go and uses a Personal Access Token (stdio mode, no OAuth).
...
- **Global (<NT>all projects</NT>)**: `~/.cursor/mcp.json`
...
4. Test with: "List my <NT>Bitrise</NT> apps" (the first tool call will trigger the OAuth flow if you're not already signed in)
...
- **<NT>OAuth flow doesn't open browser</NT>**: Update Cursor to a recent build. Older builds with Streamable HTTP but without OAuth support can use the PAT-based fallback above.
...
- **<NT>Invalid JSON</NT>**: Validate that json format is correct
- **<NT>Tools not appearing</NT>**: Check server shows green dot in MCP settings
- **<NT>Check</NT> logs**: Look for MCP-related errors in Cursor logs

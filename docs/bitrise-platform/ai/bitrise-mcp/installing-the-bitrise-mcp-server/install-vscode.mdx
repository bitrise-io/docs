...
# Install <NT>Bitrise MCP</NT> Server in VS Code

## Prerequisites
- [VS Code](https://code.visualstudio.com/Download) installed (recent version, with MCP OAuth support)
- A <NT>Bitrise</NT> account
- For local setup: [Go](https://go.dev/) (>=1.25) installed and a <NT>Bitrise</NT> PAT

## Remote Server Setup (Streamable HTTP) — Recommended

VS Code's MCP integration handles OAuth automatically. On first connection it'll open your browser to sign you in to <NT>Bitrise</NT> — no token needed.

...
Rules (same exclusion machinery as add_glossary_terms.py):
  ...
  - The following regions are never touched: frontmatter, fenced code blocks,
    ...
    anything already inside an <NT> block. Headings and admonition titles
    are otherwise NOT excluded — unlike <GlossTerm>'s tooltip (a real
    UX/anchor-generation concern inside a heading or title bar), <NT> is an
    inert translate="no" span with no visual footprint, so a glossary term
    inside a heading or admonition title gets wrapped exactly like it would
    in body text; only the `{#custom-id}` suffix itself is protected, since
    wrapping inside it produces an invalid anchor.

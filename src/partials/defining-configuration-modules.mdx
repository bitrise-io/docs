...
- Items of array type (for example, lists of Environment Variables or [trigger map items](/en/bitrise-ci/run-and-analyze-builds/build-triggers/yaml-syntax-for-build-triggers.html)): if the collection is present in both the most recently read file and the existing merged configuration, the merged value is an ordered array of values, with all values from the merged configuration followed by the values from the most recently read file.

...
Step inputs are Bitrise [Environment Variables](/en/bitrise-ci/configure-builds/environment-variables.html): they consist of a key and value pair that users of the Step can set to control the Step behavior. For example, the **Git Clone** Step has an input with the key branch:
